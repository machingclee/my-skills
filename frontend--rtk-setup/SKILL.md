...
For list-endpoint normalization (`normalizeUtil`, `ids` / `idToObject`,
`selectFromResult`), use **`frontend--rtk-normalize-list`** separately.
...
└── api/
    ...
    └── exampleApi.ts        # injectEndpoints — dummy query + mutation comments
```
...
5. If the user also needs list normalization, invoke **`frontend--rtk-normalize-list`**
   after this scaffold (adds `normalize.ts` and the list endpoint pattern).
...
For list normalization you will also need `normalizr` — see
**`frontend--rtk-normalize-list`**.
